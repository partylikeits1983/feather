...
import { fileGlyph, isTex } from '../types';
import { api } from '../api';
...
export function Explorer({ collapsed, workspace, active, revision, onOpen, onMenu, onError }: {
  ...
  const renderDirectory = (path: string, depth: number) => (cache[path] || []).map(entry => <div key={entry.path} role="none">
    <div class={`tree-row ${entry.path === active ? 'selected' : ''}`} role="none">
      <button class="tree-item" style={{ paddingLeft: `${12 + depth * 12}px` }}
        ...
        onClick={() => entry.isDir ? setExpanded(previous => { const next = new Set(previous); next.has(entry.path) ? next.delete(entry.path) : next.add(entry.path); return next; }) : onOpen(entry.path)}
        onContextMenu={event => { event.preventDefault(); onMenu(entry, event.clientX, event.clientY); }}>
        {entry.isDir ? <><Icon name="chevron" size={11} class={expanded.has(entry.path) ? 'rotated' : ''} /><Icon name="folder" size={15} /></> : <><span class="tree-spacer" /><span class={`file-glyph ${isTex(entry.path) ? 'tex' : ''}`}>{fileGlyph(entry.path)}</span></>}
  ...
  return <div id="workspace-files" class="tree" hidden={collapsed} role="tree" aria-label="Files">{renderDirectory('', 0)}{cache['']?.length === 0 && <p class="tree-empty">A fresh start.<br />Create your first file above.</p>}</div>;
}
